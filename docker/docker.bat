...
@if "%apikey%" == "EDITME" (
  ...
  if "%apikey%" == "" (
	...
	docker.exe run --detach --rm --name grima --publish %port%:19290 --volume %persist%:/home/grima/persist --env DATABASE_URL=%DATABASE_URL% "rdeiser/krima"
  ) else (
	echo Using environment variables
	docker.exe run --detach --rm --name grima --publish %port%:19290 --env apikey=%apikey% --env server=%server% "rdeiser/krima"
  )
