...
if (startswith($item['call_number'],'MAP')) {
	$pattern = '/(MAP)/';
	$replace = '';
	}
?>
              <table class="table">
				...
				<tr><th class="flip"><span>Call Number:</span><span>Número de clasificación:</span></th><td><?= preg_replace($pattern, $replace, $item['call_number'])?><text <?=$style4?>><?= preg_replace($pattern2, $replace2, $item['description'])?></text><text <?=$style3?>>&nbsp;c.<?=$e($item['copy_id'])?></text></td></tr>
